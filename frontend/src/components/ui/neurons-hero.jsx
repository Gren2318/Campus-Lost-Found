    ...
    useEffect(() => {
        ...
        class Neuron {
            ...
            draw() {
                ...
                const color = `rgba(14, 165, 233, ${0.2 + this.activation * 0.8})`; // primary-500
                ctx.fillStyle = color;
                ctx.fill();
            }
        ...
        class Pulse {
            ...
            draw() {
                ...
                ctx.fillStyle = `rgba(56, 189, 248, ${1 - this.progress})`; // primary-400
                ctx.shadowColor = '#0ea5e9'; // primary-500
                ctx.shadowBlur = 10;
                ctx.fill();
                ctx.shadowBlur = 0;
            }
...
const CampusConnectHero = ({ isLoggedIn }) => {
    ...
    return (
        ...
        >
            ...
            <div className="relative z-20 text-center p-6 w-full max-w-5xl mx-auto flex flex-col items-center">
                <motion.div
                    custom={0} variants={fadeUpVariants} initial="hidden" animate="visible"
                    className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary-500/10 border border-primary-500/20 mb-6 backdrop-blur-sm shadow-glow"
                >
                ...
                <motion.h1
                    ...
                    className="text-5xl md:text-8xl font-black font-heading tracking-tighter mb-6 text-white leading-tight"
                >
                    Lost something on <br className="hidden md:block"/><span className="text-transparent bg-clip-text bg-gradient-to-r from-primary-400 to-cyan-200">Campus?</span>
                </motion.h1>

                <motion.p
                    custom={2} variants={fadeUpVariants} initial="hidden" animate="visible"
                    className="max-w-2xl mx-auto text-lg md:text-2xl text-gray-400 mb-10 font-medium leading-relaxed"
                >
                    The smartest way to reconnect with your belongings. AI-powered lost & found for your community.
                </motion.p>
                ...
                >
                    <Link to={isLoggedIn ? "/post" : "/login"}>
                        <button className="px-8 py-4 bg-primary-600 font-bold text-white rounded-2xl shadow-glow hover:bg-primary-500 transition-all duration-300 flex items-center justify-center gap-2 w-full sm:w-auto hover:-translate-y-1">
                            {isLoggedIn ? "Report Item" : "Login"}
                            <ArrowRight className="h-5 w-5" />
                        </button>
                    </Link>
                    {!isLoggedIn && (
                        <Link to="/register">
                            <button className="px-8 py-4 bg-gray-900 border border-gray-700 text-gray-200 font-bold rounded-2xl shadow-lg hover:bg-gray-800 transition-all duration-300 w-full sm:w-auto hover:-translate-y-1">
                                Create Account
                            </button>
                        </Link>
                    )}
